...
third_position = [110, 104.78, 71.66, 87.41007232666016, 180, 1.51, 90]
fourth_position = [110, 104.78, 71.66, 87.41007232666016, 180, 1.51, 160]
fifth_position = [92, 140.2877655029297, 42.30215835571289, 87.19424438476562, 180.0, 1.5107914209365845, 160]
#
#Define the method which contains the main functionality of the node.
def controller_wo_frame():
  ...
  for i in range(pushlish_times):
    joint_cmd.header.stamp = rospy.Time.now()
    joint_cmd.position = fourth_position
    pub.publish(joint_cmd)
    r.sleep()
  print "fourth joint done"

  for i in range(pushlish_times):
    joint_cmd.header.stamp = rospy.Time.now()
    joint_cmd.position = fifth_position
    pub.publish(joint_cmd)
    r.sleep()
  print "fifth joint done"
